...
function enumExists(name, value, allEnums) {
    return allEnums.some(function (x) { return x.name === name; });
}
...
function convertType(swaggerType, swagger) {
    ...
    if (swaggerType.hasOwnProperty('schema')) {
        ...
        typespec.tsType = 'enum'
        typespec.enumValues = swaggerType.enum;
        typespec.enumInlined = swaggerType.enum.map(function (str) { return '\'' + str + '\''; }).join(' | ');
    ...
    typespec.isEnum = typespec.tsType === 'enum';
